const DQ_EXERCISES = {
    ...
    renderQuests() {
        ...
        // Füge Rest Day Box hinzu, wenn es ein Rest Day ist
        if (userGoal === 'restday') {
            const box = document.createElement('div');
            box.className = 'restday-info-box';
            box.innerHTML = DQ_DATA.translations[lang].restday_info_box;
            
            // Füge die Box direkt nach der Überschrift ein
            const questsHeading = mainQuestSection.querySelector('h2');
            if (questsHeading) {
                questsHeading.insertAdjacentElement('afterend', box);
            }
        }
