const DQ_UI = {
    ...
    // --- DUNGEON: Floating spawn chip (Phase 1) ---
    mountDungeonSpawnChipIfNeeded() {
        try {
            const always = this.getDungeonAlwaysPopup();
            const prob = this.getDungeonSpawnProbability();
            const shouldShow = always || Math.random() < prob;
            ...
            chip.addEventListener('click', () => {
                // Route zur Dungeon-View
                if (typeof DQ_DUNGEON_MAIN !== 'undefined' && DQ_DUNGEON_MAIN && DQ_DUNGEON_MAIN.open) {
                    // Entferne Chip sofort beim Betreten des Dungeons
                    try { chip.remove(); } catch {}
                    DQ_DUNGEON_MAIN.open();
                } else {
                    // Fallback: wechsle zur Exercises-Seite und zeige Info
                    ...
                    // Retry shortly until DQ_DUNGEON_MAIN is ready
                    let tries = 0;
                    const t = setInterval(() => {
                        tries++;
                        if (typeof DQ_DUNGEON_MAIN !== 'undefined' && DQ_DUNGEON_MAIN && DQ_DUNGEON_MAIN.open) {
