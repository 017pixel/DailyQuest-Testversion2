const DQ_SHOP = {
        ...
        charStore.get(1).onsuccess = (e) => {
            ...
            shopStore.getAll().onsuccess = (ev) => {
                ...
                filteredItems.forEach(item => {
                    ...
                    card.innerHTML = `<h2>${item.name}</h2><p>${item.description}</p><p>Kosten: <span class="material-symbols-rounded icon-gold">paid</span> ${item.cost}</p><button class="card-button" data-item-id="${item.id}" ${canAfford ? '' : 'disabled'}>Kaufen</button>`;
    ...
    async renderPlantShop() {
        ...
        filteredItems.forEach(item => {
            ...
            const iconName = this.emojiToIcon(item.emoji);
            card.innerHTML = `
                <div class="item-emoji"><span class="material-symbols-rounded">${iconName}</span></div>
                <h3>${item.name[lang]}</h3>
                <p class="item-price"><span class="material-symbols-rounded icon-gold">paid</span> ${item.price}</p>
