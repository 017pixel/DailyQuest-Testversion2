const DQ_FOKUS_TIMER = {
    ...
    handleStartClick() {
        ...
        if (state.linkedQuest && state.linkedQuest.labelKey) {
            const lang = DQ_CONFIG.userSettings.language || 'de';
            const label = DQ_DATA.translations[lang][state.linkedQuest.labelKey] || 'Lernen';
    ...
    async completeSession(minutes) {
        ...
        if (linkedQuest) {
            ...
            if (type === 'quest') {
                await DQ_EXERCISES.completeQuest(id);
            } else if (type === 'free') {
                await DQ_EXERCISES.completeFreeExercise(id);
            }
            return;
        }
