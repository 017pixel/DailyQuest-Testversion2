const DQ_DUNGEON_MAIN = {
    ...
    async open() {
        ...
        // Zeige Tutorial für Dungeon wenn nötig (nach Rendering)
        if (typeof DQ_TUTORIAL_PROGRESSIVE !== 'undefined') {
            // Kurze Verzögerung damit alles gerendert ist
            setTimeout(() => {
                DQ_TUTORIAL_PROGRESSIVE.showFeatureTutorial('dungeon_combat');
            }, 500);
        }
    },
        ...
        actions.querySelectorAll('.exercise-card .complete-button-small').forEach(btn => {
            btn.addEventListener('click', async (e) => {
                ...
                if (DQ_DUNGEON_COMBAT.isWin()) {
                    ...
                    // Tutorial Abschluss
                    if (typeof DQ_TUTORIAL_PROGRESSIVE !== 'undefined') {
                        await DQ_TUTORIAL_PROGRESSIVE.showFeatureTutorial('dungeon_win');
                    }
