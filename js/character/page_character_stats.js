const DQ_STATS = {
    ...
    focusChartColorPalette: [
        '#5f8575', '#6a9485', '#75a395', '#80b2a5', '#8bc1b5', 
        '#96d0c5', '#a1dfd5', '#aceee5', '#b7fdf5'
    ],
    colorIndex: 0,
    ...
    renderFocusStats() {
        ...
        if (!vibeState || !vibeState.sessions || vibeState.sessions.length === 0) {
            section.style.display = 'none';
            return;
        }
        section.style.display = 'block';

        const totalMinutes = vibeState.sessions.reduce((sum, s) => sum + s.duration, 0);
        const totalSessions = vibeState.sessions.length;
        const totalPlants = vibeState.sessions.length;
